<x-layout>
    ...
    <div class='px-5 py-3 shadow-md border'>
        ...
        <span
            class="text-sm text-gray-400 font-semibold">{{ $post->author->user->firstName . ' ' . $post->author->user->lastName }}</span>
        ...
        <ul>
            ...
            @foreach ($post->comments as $comment)
                <li class="text-xs text-gray-700">-> {{ $comment->content }} |
                    <span
                        class="text-black">{{ $comment->author->user->firstName . ' ' . $comment->author->user->lastName }}</span>
                </li>
            @endforeach
        </ul>
